...
| ------------- | ------------- | ------------- |
| [**locationAdminDelete()**](LocationAdminApi.md#locationAdminDelete) | **DELETE** /location/admin | Delete locations |
| [**locationAdminGet()**](LocationAdminApi.md#locationAdminGet) | **GET** /location/admin | Get location list |
...
| [**locationAdminNotationPost()**](LocationAdminApi.md#locationAdminNotationPost) | **POST** /location/admin/notation | Create locations with notation |
...
```php
locationAdminDelete($ids, $session): \OpenAPI\Client\Model\ModelSwagStatusOk
```

Delete locations

This endpoint is for removing locations.  _only accessible with permission_ : `\"ManageContent\"` `\"ManageProjects\"`  _fully accessible with permission_ : `\"ManageContent\"` `\"ManageProjects\"`
...
$ids = 'ids_example'; // string | location ids to be deleted
$session = 'session_example'; // string | JWT

try {
    $result = $apiInstance->locationAdminDelete($ids, $session);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LocationAdminApi->locationAdminDelete: ', $e->getMessage(), PHP_EOL;
}
```
...
| ------------- | ------------- | ------------- | ------------- |
| **ids** | **string**| location ids to be deleted | |
| **session** | **string**| JWT | |
...
```php
locationAdminGet($cursor, $page, $session, $limit): \OpenAPI\Client\Model\LocationResponseListAdmin[]
```

Get location list

This endpoint returns an array of all locations for a cursor. The output is in administrative representation and filtered for the user permissions.  Cursor could be created here: POST /location/admin/search  _accessible without permission_
...
try {
    $result = $apiInstance->locationAdminGet($cursor, $page, $session, $limit);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LocationAdminApi->locationAdminGet: ', $e->getMessage(), PHP_EOL;
}
```
...
## `locationAdminNotationPost()`

```php
locationAdminNotationPost($session, $request): \OpenAPI\Client\Model\ModelCursorResponse
```

Create locations with notation

This endpoint is for creating multiple locations with optional `{a..b:c}` notation in title. In case of response status is 205 the request was only executed partial. Use GET to receive the stored configuration.  `a` is the number of the first location and must be smaller than `b` in valid notation  `b` is the number of the last location and must be bigger than `a` and the difference between `a` and `b` does not be bigger than 100 in valid notation  `:c` is the optional pad to add leading zeros to `a` and `b` and must be bigger than `0` in valid notation. e.g.: {-1..10:3} -> -01..010  __Note:__ if no valid notation is found, a single location with the provided title is created.  _only accessible with permission_ : `\"ManageContent\"` `\"ManageProjects\"`  _fully accessible with permission_ : `\"ManageContent\"` `\"ManageProjects\"`
...
$session = 'session_example'; // string | JWT
$request = new \OpenAPI\Client\Model\LocationRequestAdmin(); // \OpenAPI\Client\Model\LocationRequestAdmin | Locations to create

try {
    $result = $apiInstance->locationAdminNotationPost($session, $request);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LocationAdminApi->locationAdminNotationPost: ', $e->getMessage(), PHP_EOL;
}
```
...
| ------------- | ------------- | ------------- | ------------- |
| **session** | **string**| JWT | |
| **request** | [**\OpenAPI\Client\Model\LocationRequestAdmin**](../Model/LocationRequestAdmin.md)| Locations to create | |
...
$session = 'session_example'; // string | JWT
$request = new \OpenAPI\Client\Model\LocationCursorRequestAdmin(); // \OpenAPI\Client\Model\LocationCursorRequestAdmin | options to create cursor

try {
    $result = $apiInstance->locationAdminSearchPost($session, $request);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LocationAdminApi->locationAdminSearchPost: ', $e->getMessage(), PHP_EOL;
}
```
...
| ------------- | ------------- | ------------- | ------------- |
| **session** | **string**| JWT | |
| **request** | [**\OpenAPI\Client\Model\LocationCursorRequestAdmin**](../Model/LocationCursorRequestAdmin.md)| options to create cursor | |
